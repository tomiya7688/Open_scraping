...
FlowConfigの正式契約は `0.3`。機械可読スキーマと検証器は `packages/contracts` に実装する。HTTP API 自体は後続Issueで実装する。詳細は [contracts.md](contracts.md)。
...
`nodes`は`id`、`component`、`operation`、`inputs`、任意の`policy`を持つ。依存関係は入力参照から導く。0.3 は非循環DAGのみを受理する。`branch`、`repeat`、`while`、`for_each`、`pagination` 等の制御構文は0.3では未提供で、未知フィールドとして明示的に拒否する。有限のページ送り・バッチは、上限を設定した部品操作の内部処理として扱う。将来DSLへ制御構文を追加する場合は schema_version を更新する。
...
例ファイルは FlowConfig 0.3 の正式スキーマで検証する。フローランタイムや例中の実処理部品が実装済みであることは意味しない。
